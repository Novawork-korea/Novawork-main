/* ==================== EnhanceV8: directional reveal + FAQ search/category ==================== */
(() => {
  "use strict";

  const root = document.documentElement;
  if (root.dataset.nwInteractionV8 === "ready") return;
  root.dataset.nwInteractionV8 = "ready";
  root.dataset.nwInteractionV7 = "superseded-v8";
  window.__NW_INTERACTION_PATCH = "EnhanceV8";
  ...
  const initFaqSearchV8 = () => {
    if (root.dataset.nwFaqSearchV8 === "ready") return;
    const body = document.body;
    if (!body || !body.classList.contains("faq-static-page")) return;
    const section = document.querySelector(".faq-section");
    const container = section ? section.querySelector(":scope > .container") : null;
    if (!container) return;
    root.dataset.nwFaqSearchV8 = "ready";

    const pairs = [];
    const children = Array.from(container.children);
    for (let i = 0; i < children.length; i += 1) {
      const head = children[i];
      if (!(head instanceof HTMLElement) || !head.classList.contains("faq-category-head")) continue;
      const list = children[i + 1];
      if (!(list instanceof HTMLElement) || !list.classList.contains("faq-list")) continue;
      const title = head.querySelector(".faq-category-title");
      const label = normalizeText(title ? title.textContent : head.textContent) || "faq";
      const id = "faq-cat-" + String(pairs.length + 1).padStart(2, "0");
      head.dataset.faqCategory = id;
      list.dataset.faqCategory = id;
      pairs.push({ id, label, head, list, title: title ? title.textContent.trim() : label });
    }
    ...
    empty.hidden = true;
    empty.innerHTML = '<strong>검색 결과가 없습니다.</strong><p>다른 키워드로 검색하거나 전체 카테고리에서 다시 확인해 주세요.</p>';
    ...
    pairs.forEach((pair) => {
      Array.from(pair.list.querySelectorAll(".faq-item")).forEach((item) => {
        ...
        const answer = Array.from(item.querySelectorAll("p, li")).map((node) => node.textContent || "").join(" ");
        const text = normalizeText([pair.title, summary ? summary.textContent : "", answer].join(" "));
    ...
    const applyFilter = () => {
      ...
      items.forEach(({ item, pair, text, wasOpen }) => {
        const categoryMatch = activeCategory === "all" || pair.id === activeCategory;
        ...
        if (visible) {
          visibleCount += 1;
          visibleByCategory.set(pair.id, (visibleByCategory.get(pair.id) || 0) + 1);
  ...
  const initDirectionalRevealV8 = () => {
    if (root.dataset.nwScrollRevealV8 === "ready") return;
    const body = document.body;
    if (!body) return;
    const pathname = window.location.pathname || "";
    const isContact = body.classList.contains("contact-static-page") || /(?:^|\/)contact\.html(?:$|[?#])/i.test(window.location.href) || /(?:^|\/)contact\.html$/i.test(pathname);
    if (isContact) return;
    root.dataset.nwScrollRevealV8 = "ready";
    ...
    const rejectClassPart = /(grid|list|head|body|copy|title|desc|label|actions|nav|menu|footer|header|field|input|toggle|shortcut|business|note|meta|flow)$/i;
    const atomicClass = (el) => Array.from(el.classList || []).some((name) => {
      if (rejectClassPart.test(name)) return false;
      return /(^|-)card$|(^|-)box$|(^|-)panel$|(^|-)row$|(^|-)step$|(^|-)signal$|(^|-)item$|(^|-)visual$|(^|-)band$/i.test(name);
    });
    ...
    const add = (el) => {
      ...
      if (el.dataset.nwReveal === "skip" || el.dataset.nwRevealV8 === "skip") return;
    ...
    items.forEach((el, index) => {
      ...
      const seed = hashText([el.tagName, el.className, el.textContent.slice(0, 64), index].join("|"));
      const direction = directions[seed % directions.length];
      el.dataset.nwRevealDirection = direction;
      el.style.setProperty("--nw-reveal-delay", Math.min(siblingIndex * 44, 176) + "ms");
      el.dataset.nwRevealV8 = isInFirstView(el) ? "visible" : "pending";
      if (el.dataset.nwRevealV8 === "visible") el.dataset.nwRevealV8Complete = "true";
    });

    body.classList.add("nw-scroll-reveal-v8");

    const markVisible = (el) => {
      if (!(el instanceof HTMLElement) || el.dataset.nwRevealV8 === "visible") return;
      el.dataset.nwRevealV8 = "visible";
      window.setTimeout(() => {
        if (el.dataset.nwRevealV8 === "visible") {
          el.dataset.nwRevealV8Complete = "true";
    ...
    const refresh = () => {
      const height = vh();
      items.forEach((el) => {
        if (!(el instanceof HTMLElement) || el.dataset.nwRevealV8 === "visible" || el.hidden) return;
        const rect = el.getBoundingClientRect();
    ...
    document.addEventListener("novawork:route-enter-done", () => raf(observePending), { passive: true });
    window.addEventListener("pageshow", () => raf(observePending), { passive: true });
    window.addEventListener("resize", () => raf(refresh), { passive: true });
    window.addEventListener("scroll", () => raf(refresh), { passive: true });
    document.addEventListener("input", (event) => {
      if (event.target && event.target.closest && event.target.closest(".faq-tools")) raf(refresh);
    }, { passive: true });
  };
