(() => {
  ...
  if (root.dataset.nwCore === "ready") return;
  root.dataset.nwCore = "ready";
  ...
  const initCurrentPage = () => {
    ...
    const current = currentRaw.startsWith("service-")
      ? "services.html"
      : currentRaw;
    document
      .querySelectorAll(
        ".nav-link, .mobile-menu-list a, .mobile-menu-shortcut-grid a, .footer-nav a",
      )
      .forEach((link) => {
        ...
        if (normalizedHref === current || normalizedHref === currentRaw) {
  ...
  const initMobileMenu = () => {
    ...
    const syncMenuGeometry = () => {
      ...
      menu.style.setProperty(
        "--nw-menu-toggle-width",
        Math.max(36, Math.round(rect.width || 42)) + "px",
      );
    };
    ...
    const clearRouteStateForMenu = () => {
      ...
      document
        .querySelectorAll(".nw-route-curtain")
    ...
    const focusable = () =>
      Array.from(menu.querySelectorAll(focusableSelector)).filter((el) => {
    ...
    const lockBodyScroll = () => {
      if (!shouldLockBody() || body.dataset.nwMenuScrollLocked === "true")
        return;
    ...
    const setOpen = (nextOpen, options = {}) => {
      const staleState =
        body.classList.contains("menu-open") !== nextOpen ||
        root.classList.contains("menu-open") !== nextOpen;
    ...
    window.addEventListener("orientationchange", closeForWideLayout, {
      passive: true,
    });
  ...
  const initTracking = () => {
    ...
    document.addEventListener("click", (event) => {
      ...
      if (href.startsWith("tel:"))
        return sendEvent("click_phone", { method: "phone", link_area: area });
      if (href.startsWith("mailto:"))
        return sendEvent("click_email", { method: "email", link_area: area });
      if (href.includes("pf.kakao.com"))
        return sendEvent("click_kakao", {
          method: "kakao",
      ...
      if (href.includes("contact.html") || href.includes("#inquiry-form")) {
        sendEvent("cta_click", {
          link_text: label,
          link_area: area,
          destination: href.includes("#inquiry-form")
            ? "inquiry_form"
            : "contact_page",
        });
      }
  ...
  const initFloatingActions = () => {
    if (root.dataset.nwFloatingActions === "ready") return;
    root.dataset.nwFloatingActions = "ready";
    ...
    const contactHref = pageSpecificContact
      ? pageSpecificContact.getAttribute("href") || "contact.html"
      : "contact.html";
    ...
    let heroReady =
      !isHome() ||
      Boolean(document.querySelector(".nw-logo-scroll.is-intro-complete"));
    const isSuppressed = () =>
    ...
    document.addEventListener("novawork:hero-intro-start", onHeroStart, {
      passive: true,
    });
    window.addEventListener("novawork:hero-intro-start", onHeroStart, {
      passive: true,
    });
    document.addEventListener("novawork:hero-intro-done", onHeroDone, {
      passive: true,
    });
    window.addEventListener("novawork:hero-intro-done", onHeroDone, {
      passive: true,
    });
    document.addEventListener("novawork:route-enter-done", request, {
      passive: true,
    });
    window.addEventListener("novawork:route-enter-done", request, {
      passive: true,
    });
    window.addEventListener("scroll", request, { passive: true });
  ...
  const initServiceDetailAccordions = () => {
    if (
      ...
      root.dataset.nwDetailAccordions === "ready"
    )
      return;
    root.dataset.nwDetailAccordions = "ready";

    const panels = Array.from(
      document.querySelectorAll(".detail-main .detail-panel"),
    );
    ...
    const openPanelById = (id, animate = true) => {
      ...
      if (panel && typeof panel.nwSetAccordionOpen === "function")
        panel.nwSetAccordionOpen(true, animate);
    };
    ...
    panels.forEach((panel, index) => {
      ...
      if (!toggle) {
        ...
        toggle.innerHTML =
          '<span class="visually-hidden">내용 펼치기</span><i aria-hidden="true"></i>';
      ...
      setOpen(
        window.location.hash && window.location.hash.slice(1) === panel.id,
        false,
      );
    ...
    if (window.location.hash)
      window.setTimeout(
        () => openPanelById(window.location.hash.slice(1), true),
        120,
      );
  };
